...
/****** Object:  StoredProcedure [dbo].[GET_THUADAT_BYSOTO_DDQG]    Script Date: 6/10/2021 2:17:52 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GET_THUADAT_BYSOTO_DDQG]
   
   @maxa varchar(10),
   @sohieutobando nvarchar(50),
   @pageSize INT,
   @pageIndex INT
AS
     declare @MaDVHC int
     set @MaDVHC=100000 + cast(@maxa as int)
     ...
     declare @tblThuaDat table(Ma bigint IDENTITY(1,1) PRIMARY KEY, SoToSX nvarchar(50), MaThuaDat bigint, SoTo nvarchar(50),
							   ...
							   LoaiTaiSan nvarchar(200), MaGCN bigint, MaDangKy bigint)
	 
		 insert into @tblThuaDat(SoToSX, MaThuaDat, SoTo, ThuaDatSo, DTPhapLy, MucDichSD, NguonGocSD,
								 TrangThai, SoSerial, HanCheSDD, LoaiTaiSan, MaGCN, MaDangKy)

		 select SoToSX, MaThuaDat,  SoTo,
				ThuaDatSo, DTPhapLy, 

				(
				   case (select count(*) from DangKyMDSDD where MaGCN=MaGCN) 
				   when 1 then (
				...
				(
				   case (select count(*) from DangKyNGSDD where MaGCN=MaGCN) 
				   when 1 then (
				...
				'' as TrangThai, SoSerial, 

				(
				   case (select loaiHanChe from DC_HanCheSuDungDat where (MaGCN=MaGCN)and(ltrim(rtrim(soVanBanPhapLy))<>''))
				...
				'' as LoaiTaiSan, MaGCN, MaDangKy
	        
		 from (select  tbd.SoTo as SoToSX, td.MaThuaDat,(ltrim(rtrim(tbd.SoTo)) + ' (' + ltrim(rtrim(tbd.TyLe)) + ')') as SoTo,
			td.ThuaDatSo, ROUND((gcn.DienTichChung + gcn.DienTichRieng), 1) as DTPhapLy,'' as TrangThai, gcn.SoSerial, '' as LoaiTaiSan, gcn.MaGCN, dk.MaDangKy
				from ThuaDat td
				   join ToBanDo tbd on tbd.MaToBanDo = td.MaToBanDo and tbd.MaDVHC=@MaDVHC and (ltrim(rtrim(upper(tbd.SoTo)))=ltrim(rtrim(upper(@sohieutobando))))
				   join DangKyQSDD dk on dk.MaThuaDat = td.MaThuaDat 
				   join GCNQSDD gcn on gcn.MaDangKy = dk.MaDangKy) T	       
     ...
     select ltrim(rtrim(str(MaThuaDat))) as id, SoTo as sohieutobando, ThuaDatSo as sothututhua, @maxa as maxa,
