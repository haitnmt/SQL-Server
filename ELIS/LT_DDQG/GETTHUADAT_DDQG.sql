...
/****** Object:  StoredProcedure [dbo].[GETTHUADAT_DDQG]    Script Date: 6/10/2021 2:18:05 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GETTHUADAT_DDQG]
   @maxa varchar(10),
   @sohieutobando nvarchar(50),
   @sothututhua nvarchar(50)
AS
     declare @MaDVHC int
	 ...
	 select tbd.SoTo as SoToSX, td.MaThuaDat, (ltrim(rtrim(tbd.SoTo)) + ' (' + ltrim(rtrim(tbd.TyLe)) + ')') as SoTo,
			...
			'' as LoaiTaiSan, gcn.MaGCN, dk.MaDangKy
	        
	 from ToBanDo tbd, ThuaDat td, DangKyQSDD dk, GCNQSDD gcn
	 where (tbd.MaDVHC=@MaDVHC)and(ltrim(rtrim(upper(tbd.SoTo)))=ltrim(rtrim(upper(@sohieutobando))))and
	       (ltrim(rtrim(upper(td.ThuaDatSo)))=ltrim(rtrim(upper(@sothututhua))))and
	       (tbd.MaToBanDo=td.MaToBanDo)and(td.MaThuaDat=dk.MaThuaDat)and(dk.MaDangKy=gcn.MaDangKy)	       
